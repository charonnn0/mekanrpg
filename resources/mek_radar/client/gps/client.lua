...
gpsSoundMuted = exports.mek_settings:getPlayerSetting(localPlayer, "gps_sound_muted")
gpsRoute = false
gpsThread = false
gpsWaypoints = {}
nextWp = false
...
addEvent("vehicleGPS.syncClient", true)
addEventHandler("vehicleGPS.syncClient", root, function(data)
	setElementData(source, "gps_destination", data, false)
end)

addEventHandler("onClientResourceStop", resourceRoot, function()
	triggerEvent("settings.patch", localPlayer, "gps_sound_muted", gpsSoundMuted)
end)
...
addEventHandler("onClientResourceStart", getResourceRootElement(), function()
	...
	if occupiedVehicle then
		if getElementData(occupiedVehicle, "gps_destination") then
			local destination = getElementData(occupiedVehicle, "gps_destination")
...
addCommandHandler("tognodes", function()
...
function isVehicleOnAsphalt(vehicle)
	local vehPosX, vehPosY, vehPosZ = getElementPosition(vehicle)
	local areaID = math.floor((vehPosY + 3000) / 750) * 8 + math.floor((vehPosX + 3000) / 750)

	local nodes = vehicleNodes[areaID]
	local node = getVehicleNodeClosestToPoint(vehPosX, vehPosY, vehPosZ, nodes)

	local distance = getDistanceBetweenPoints3D(vehPosX, vehPosY, vehPosZ, node.x, node.y, node.z)
...
function makeRoute(destinationX, destinationY, uTurned)
	...
	if not occupiedVehicle then
		if occupiedPlayer then
			occupiedVehicle = occupiedPlayer
		else
			return
		end
	end
	...
	if not routePath then
		setElementData(occupiedVehicle, "gps_destination", false, true)
		return false
	end
	...
	for i, node in ipairs(gpsRoute) do
		...
		if i == #gpsRoute then
			...
			targetBlip = createBlip(node.x, node.y, node.z, 41)
		end
	end
	...
	for turn = 1, #waypointTurns do
		...
		local distanceBetweenWaypoints = 0
			+ getDistanceBetweenPoints2D(
				gpsRoute[nextTurnNode].x,
				gpsRoute[nextTurnNode].y,
				gpsRoute[waypointTurns[nextTurnId][1]].x,
				gpsRoute[waypointTurns[nextTurnId][1]].y
			)
				...
				if
					0
						+ getDistanceBetweenPoints2D(
							gpsRoute[i].x,
							gpsRoute[i].y,
							gpsRoute[nextTurnNodeId].x,
							gpsRoute[nextTurnNodeId].y
						)
					> distanceBetweenWaypoints - 500
				then
	...
	for i = 1, tonumber(gpsWaypoints[nextWp][1]) or #gpsRoute do
		gpsWaypoints[nextWp][3] = getDistanceBetweenPoints2D(gpsRoute[i].x, gpsRoute[i].y, gpsRoute[1].x, gpsRoute[1].y)
			/ 0.9
	end

	local vehicleOffsetX, vehicleOffsetY = getPositionFromElementOffset(occupiedVehicle, -1, 0, 0)
	local vehicleAngle = math.deg(
		getAngle(
			gpsRoute[2].x - gpsRoute[1].x,
			gpsRoute[2].y - gpsRoute[1].y,
			vehicleOffsetX - vehiclePosX,
			vehicleOffsetY - vehiclePosY
		)
	)
	...
	if vehicleAngle > 0 then
		...
		if not uTurned then
			currentGPSSound = setTimer(playGPSSound, 1750, 1, "UTURN")
		else
			playGPSSound("UTURN")
		end
...
addEventHandler("onClientColShapeHit", root, function(element)
	if colshapeElements[source] and element == localPlayer then
		...
		if currentShape == #gpsRoute then
			...
			checkForRerouteTimer = false
			--triggerServerEvent("vehicleGPS.syncPure", localPlayer, occupiedVehicle, false, getVehicleOccupants(occupiedVehicle))
			setElementData(occupiedVehicle, "gps_destination", false, true)
			setElementData(localPlayer, "gps_destination", false, true)
			return
		else
			for i = 1, currentShape do
				if isElement(gpsColshapes[i]) then
		...
		if gpsWaypoints[nextWp] and gpsWaypoints[nextWp][1] ~= "finish" then
			if currentShape >= gpsWaypoints[nextWp][1] then
				...
				for i = currentShape, tonumber(gpsWaypoints[nextWp][1]) or #gpsRoute do
					gpsWaypoints[nextWp][3] = getDistanceBetweenPoints2D(
						gpsRoute[i].x,
						gpsRoute[i].y,
						gpsRoute[currentShape].x,
						gpsRoute[currentShape].y
					) / 0.9
				end
			else
				gpsWaypoints[nextWp][3] = getDistanceBetweenPoints2D(
					gpsRoute[currentShape].x,
					gpsRoute[currentShape].y,
					gpsRoute[gpsWaypoints[nextWp][1]].x,
					gpsRoute[gpsWaypoints[nextWp][1]].y
				) / 0.9
				...
				if nextWaypointDistance > 230 and nextWaypointDistance <= 250 and not routeInstructions[250] then
					...
					if gpsWaypoints[nextWp][2] == "left" then
						playGPSSound("300;YARDS;LEFT;")
					elseif gpsWaypoints[nextWp][2] == "right" then
						playGPSSound("300;YARDS;RIGHT;")
					end
				...
				if nextWaypointDistance > 480 and nextWaypointDistance <= 500 and not routeInstructions[500] then
					...
					if gpsWaypoints[nextWp][2] == "left" then
						playGPSSound("500;YARDS;LEFT;")
					elseif gpsWaypoints[nextWp][2] == "right" then
						playGPSSound("500;YARDS;RIGHT;")
					end
...
addEventHandler("onClientVehicleEnter", root, function(player)
	if player == localPlayer and getElementData(source, "gps_destination") then
		local destination = getElementData(source, "gps_destination")
		gpsThread = coroutine.create(makeRoute)
		coroutine.resume(gpsThread, destination[1], destination[2], true)
	end
end)
...
addEventHandler("onClientElementDestroy", root, function()
	if source == occupiedVehicle and getElementData(source, "gps_destination") then
		setElementData(source, "gps_destination", false, false)
...
function calculateRoute(x1, y1, x2, y2)
	...
	if path then
		for i = 1, #path - 1 do
			if (i % 2) == 1 then
				clipboardString = clipboardString .. "{" .. path[i].id .. "," .. path[i + 1].id .. "}, \n"
			end
		end
		return path
	end
...
function checkForReroute()
	...
	local vehiclePosX, vehiclePosY = getElementPosition(occupiedVehicle)
	local nextColshapeDistance =
		getDistanceBetweenPoints2D(gpsRoute[currentNode].x, gpsRoute[currentNode].y, vehiclePosX, vehiclePosY)

	if
		nextColshapeDistance >= 30
		and nextColshapeDistance < 80
		and gpsRoute[currentNode + 1]
		and turnAroundCheckTick
		and getTickCount() - turnAroundCheckTick > 5000
	then
		local vehicleOffsetX, vehicleOffsetY = getPositionFromElementOffset(occupiedVehicle, -1, 0, 0)
		local vehicleAngle = math.deg(
			getAngle(
				gpsRoute[currentNode + 1].x - gpsRoute[currentNode].x,
				gpsRoute[currentNode + 1].y - gpsRoute[currentNode].y,
				vehicleOffsetX - vehiclePosX,
				vehicleOffsetY - vehiclePosY
			)
		)
...
function playGPSSound(sounds)
	...
	currentGPSSound = playSound("public/sounds/gps/ding.mp3")
	setSoundVolume(currentGPSSound, 0.3)
	currentGPSSoundTimer =
		setTimer(playNextGPSSound, getSoundLength(currentGPSSound) * 0.9 * 1000, 1, split(sounds, ";"), 1)
end

function playNextGPSSound(sounds, count)
	currentGPSSound = playSound("public/sounds/gps/" .. sounds[count] .. ".wav")
	setSoundVolume(currentGPSSound, 0.5)

	if count < #sounds then
		currentGPSSoundTimer =
			setTimer(playNextGPSSound, getSoundLength(currentGPSSound) * 0.9 * 1000, 1, sounds, count + 1)
	end
end
...
function playOneGPSSound(sound)
	...
	currentGPSSound = playSound("public/sounds/gps/" .. sound .. ".wav")
	setSoundVolume(currentGPSSound, 0.5)
end
