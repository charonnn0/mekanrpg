...
function attachElementToBone(element, ped, bone, x, y, z, rx, ry, rz)
	if not (isElement(element) and isElement(ped)) then
		return false
	end
	...
	x, y, z, rx, ry, rz =
		tonumber(x) or 0, tonumber(y) or 0, tonumber(z) or 0, tonumber(rx) or 0, tonumber(ry) or 0, tonumber(rz) or 0
	...
	if script_serverside then
		triggerClientEvent(root, "bones.attach", root, element, ped, bone, x, y, z, rx, ry, rz)
	end
	return true
end
...
function detachElementFromBone(element)
	...
	if script_serverside then
		triggerClientEvent(root, "bones.detach", root, element)
	end
	return true
end
...
function getElementBoneAttachmentDetails(element)
	...
	return attached_ped[element],
		attached_bone[element],
		attached_x[element],
		attached_y[element],
		attached_z[element],
		attached_rx[element],
