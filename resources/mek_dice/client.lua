function showGUI(partnerName, price)
	...
	guiGridListSetItemText(grid, 1, 1, ">> Bahis: ₺" .. exports.mek_global:formatMoney(price), false, false)
	...
	addEventHandler("onClientKey", root, function(button, state)
		if isElement(window) then
			if button == "enter" then
				triggerServerEvent("dice.spin", localPlayer)
	...
	addEventHandler("onClientGUIClick", root, function(b)
		if b == "left" then
			...
			elseif source == close_ then
				destroyElement(window)
				triggerServerEvent("dice.remove", localPlayer)
			end
		end
	end)
end
addEvent("dice.showGUI", true)
addEventHandler("dice.showGUI", root, showGUI)
...
function inviteGUI(partnerName, price)
	...
	addEventHandler("onClientGUIClick", root, function(b)
		if b == "left" then
			if source == ok then
				triggerServerEvent("dice.invite", localPlayer, localPlayer, "davet", "kabul")
				destroyElement(window)
			elseif source == deny then
				triggerServerEvent("dice.invite", localPlayer, localPlayer, "davet", "red")
				destroyElement(window)
			end
		end
	end)
end
addEvent("dice.inviteGUI", true)
addEventHandler("dice.inviteGUI", root, inviteGUI)

addEvent("dice.closeGUI", true)
addEventHandler("dice.closeGUI", root, function()
	if isElement(window) then
		destroyElement(window)
	end
end)
