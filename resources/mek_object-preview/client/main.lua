...
function objectPreview:create(
	...
	if tempRadius > new.elementRadius then
		new.elementRadius = tempRadius
	end
	...
	if new.isSecondRT then
		...
		new.shader[1] = dxCreateShader("public/shaders/fx_pre_" .. elementType .. ".fx", 0, 0, false, "all")
		if not glRenderTarget then
			glRenderTarget = dxCreateRenderTarget(scx, scy, true)
		end
	else
		new.shader[1] = dxCreateShader("public/shaders/fx_pre_" .. elementType .. "_noMRT.fx", 0, 0, false, "all")
	end
...
function objectPreview:createTextureReplace(texElement, texName)
	...
	if self.isSecondRT then
		...
		self.shader[elementNr] =
			dxCreateShader("public/shaders/fx_pre_" .. self.elementType .. "_replace.fx", 0, 0, false, "all")
		if not glRenderTarget then
			glRenderTarget = dxCreateRenderTarget(scx, scy, true)
		end
	else
		self.shader[elementNr] =
			dxCreateShader("public/shaders/fx_pre_" .. self.elementType .. "_replace_noMRT.fx", 0, 0, false, "all")
	end
...
function objectPreview:destroyTextureReplace(elementNr)
	local renID = self.renID
	if not refOPTable[renID].enabled then
		return false
	end
	if self.shader then
...
function objectPreview:update()
	...
	local posX, posY, posZ = getPositionFromMatrixOffset(
		cameraMatrix,
		{ posTemp[1], 1.6 * self.elementRadius + distTemp + posTemp[2], posTemp[3] }
	)
		...
		for i, v in ipairs(self.shader) do
			if isElement(v) then
				...
				dxSetShaderValue(
					v,
					"sScaleObject2D",
					2 * math.min(projSizeX, projSizeY),
					2 * math.min(projSizeX, projSizeY)
				)
...
function getCamVelocity()
	...
	lastCamVelocity =
		{ currentCamPos[1] - lastCamPos[1], currentCamPos[2] - lastCamPos[2], currentCamPos[3] - lastCamPos[3] }
...
function objectPreview:saveToFile(filePath)
	...
	if glRenderTarget then
		...
		projPosX, projPosY, projSizeX, projSizeY =
			toint(projPosX * scx), toint(projPosY * scy), toint(projSizeX * scx), toint(projSizeY * scy)
...
function objectPreview:drawRenderTarget()
	if not isMRTShaderSupported or not self.isSecondRT then
		return false
	end
	if glRenderTarget and self.visible and not self.attached then
		local projPosX, projPosY, projSizeX, projSizeY, postGui, isRelative = unpack(self.projection)
		projPosX, projPosY, projSizeX, projSizeY = projPosX * scx, projPosY * scy, projSizeX * scx, projSizeY * scy
		return dxDrawImageSection(
			projPosX,
			projPosY,
			projSizeX,
			projSizeY,
