...
function colorPicker.create(id, start, title)
	...
	cp.gui.window = guiCreateWindow(
		(sw - cp.gui.width) / 2,
		(sh - cp.gui.height) / 2,
		cp.gui.width,
		cp.gui.height,
		tostring(title or "Colorblender"),
		false
	)
	cp.gui.svmap = guiCreateStaticImage(16, 32, 256, 256, "public/images/blank.png", false, cp.gui.window)
	cp.gui.hbar = guiCreateStaticImage(288, 32, 32, 256, "public/images/blank.png", false, cp.gui.window)
	cp.gui.blank = guiCreateStaticImage(336, 32, 64, 64, "public/images/blank.png", false, cp.gui.window)
	cp.gui.edithex = guiCreateEdit(338, 83, 64, 20, cp.color.hex:sub(2), false, cp.gui.window)
	...
	cp.gui.okb = guiCreateButton(336, 235, 64, 24, "Tamam", false, cp.gui.window)
	cp.gui.closeb = guiCreateButton(336, 265, 64, 24, "İptal", false, cp.gui.window)
	cp.gui.focus = true
	guiWindowSetSizable(cp.gui.window, false)
	guiSetProperty(cp.gui.edithex, "ValidationString", "[0-9A-Fa-f]{0,6}")

	cp.handlers = {}
	cp.handlers.mouseDown = function()
		cp:mouseDown()
	end
	cp.handlers.mouseSnap = function()
		cp:mouseSnap()
	end
	cp.handlers.mouseUp = function(b, s)
		cp:mouseUp(b, s)
	end
	cp.handlers.mouseMove = function(x, y)
		cp:mouseMove(x, y)
	end
	cp.handlers.render = function()
		cp:render()
	end
	cp.handlers.guiFocus = function()
		cp:guiFocus()
	end
	cp.handlers.guiBlur = function()
		cp:guiBlur()
	end
	cp.handlers.pickColor = function()
		cp:pickColor()
	end
	cp.handlers.destroy = function()
		cp:customDestroy()
	end
	cp.handlers.fromText = function()
		cp:fromText()
	end
	...
	addEventHandler("onClientGUIChanged", cp.gui.edithex, cp.handlers.fromText, false)
	showCursor(true)
	return cp
end
...
function colorPicker:render()
	...
	dxDrawImage(x + 16, y + 32, 256, 256, "public/images/sv.png", 0, 0, 0, self.color.white, self.gui.focus)
	dxDrawImage(x + 288, y + 32, 32, 256, "public/images/h.png", 0, 0, 0, self.color.white, self.gui.focus)
	dxDrawImageSection(
		x + 8 + math.floor(256 * self.color.s),
		y + 24 + (256 - math.floor(256 * self.color.v)),
		16,
		16,
		0,
		0,
	...
	dxDrawImageSection(
		x + 280,
		y + 24 + (256 - math.floor(256 * self.color.h)),
		48,
		16,
		16,
		0,
	...
	dxDrawText(
		...
		x + 400,
		y + 96,
		self.color.v < 0.5 and self.color.white or self.color.black,
		1,
		"default",
		"center",
		"center",
...
function colorPicker:mouseSnap()
	if self.gui.track and source == self.gui.track then
		if self.color.s < self.gui.snaptreshold or self.color.s > 1 - self.gui.snaptreshold then
			self.color.s = math.round(self.color.s)
		end
		if self.color.v < self.gui.snaptreshold or self.color.v > 1 - self.gui.snaptreshold then
			self.color.v = math.round(self.color.v)
		end
		self:updateColor()
	end
end
...
function colorPicker:updateColor()
	...
	guiSetText(self.gui.edithex, tostring(self.color.hex):sub(2):lower())
end

function colorPicker:fromText()
	local str = guiGetText(self.gui.edithex)
	if not str or #str ~= 6 then
		return
	end

	local r, g, b = getColorFromString("#" .. str)
	if r == self.color.r and g == self.color.g and b == self.color.b then
		return
	end
