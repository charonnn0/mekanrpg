...
addCommandHandler("fp", toggleCockpitView, false, false)
...
function updateCamera()
	if isEnabled then
		...
		local camPosX, camPosY, camPosZ =
			(camPosXr + camPosXl) / 2, (camPosYr + camPosYl) / 2, (camPosZr + camPosZl) / 2
		local roll = 0
		...
		if inVehicle then
			...
			if
				getPedControlState(localPlayer, "vehicle_look_behind")
				or (
					getPedControlState(localPlayer, "vehicle_look_right")
					and getPedControlState(localPlayer, "vehicle_look_left")
				)
			then
...
function freecamMouse(cX, cY, aX, aY)
	...
	if rotX > math.pi then
		rotX = rotX - 2 * math.pi
	elseif rotX < -math.pi then
		rotX = rotX + 2 * math.pi
	end

	if rotY > math.pi then
		rotY = rotY - 2 * math.pi
	elseif rotY < -math.pi then
		rotY = rotY + 2 * math.pi
	end

	if isPedInVehicle(localPlayer) then
		if rotY < -math.pi / 4 then
			rotY = -math.pi / 4
		elseif rotY > -math.pi / 15 then
			rotY = -math.pi / 15
		end
	else
		if rotY < -math.pi / 4 then
			rotY = -math.pi / 4
		elseif rotY > math.pi / 2.1 then
			rotY = math.pi / 2.1
		end
	end
end
