...
function fallProtection(intx, inty, intz)
	...
	if
		((intz - z) > 5 or getPedSimplestTask(localPlayer) == "TASK_SIMPLE_IN_AIR")
		and getElementData(localPlayer, "logged")
		and dimension ~= 0
	then
		exports.mek_infobox:addBox("warning", "Düşme algılandı! Mülk girişine ışınlanıyorsun...")
		triggerServerEvent("fallProtectionRespawn", localPlayer, intx, inty, intz)
	end
end
