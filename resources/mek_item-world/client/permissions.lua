...
function itemPropertiesGUI(object)
	...
	if canEditItemProperties(localPlayer, object) then
		...
		local itemName = tostring(exports.mek_item:getItemName(itemID))
		...
		GUIEditor.button[4] = guiCreateButton(158, 354, 164, 27, "Kaydet", false, propertiesWindow)
		GUIEditor.button[5] = guiCreateButton(9, 354, 147, 27, "İptal", false, propertiesWindow)
		...
		addEventHandler("onClientGUIClick", GUIEditor.button[1], function()
			...
			local type
			local text =
				guiComboBoxGetItemText(GUIEditor.combobox[num], guiComboBoxGetSelected(GUIEditor.combobox[num]) or 1)
		...
		addEventHandler("onClientGUIComboBoxAccepted", GUIEditor.combobox[1], function()
			local num = 1
		...
		addEventHandler("onClientGUIComboBoxAccepted", GUIEditor.combobox[2], function()
			local num = 2
			local combo = GUIEditor.combobox[num]
			local btn = GUIEditor.button[num]
			local name = guiComboBoxGetItemText(combo, guiComboBoxGetSelected(combo))
		...
		addEventHandler("onClientGUIComboBoxAccepted", GUIEditor.combobox[3], function()
			local num = 3
			local combo = GUIEditor.combobox[num]
			local btn = GUIEditor.button[num]
			local name = guiComboBoxGetItemText(combo, guiComboBoxGetSelected(combo))
		...
		GUIEditor.label[8] = guiCreateLabel(
			9,
			239,
			296,
			56,
			"These settings define who can use, move and pick up this item. The character that dropped the item, admins and anyone with key to the interior can edit these settings. Items set as 'protected' by an admin cannot be moved or picked up before the item has been unprotected by an admin.",
			false,
			GUIEditor.tab[2]
		)
...
function showDataSet(type, action, oldData)
	...
	elseif type == 5 then --characters
		...
		local btnSave = guiCreateButton(10, 401, 366, 31, "Kaydet", false, dataWindow)
