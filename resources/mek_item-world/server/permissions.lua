function getItemPropertiesData(object)
	...
	if creator > 0 then
		creatorName = exports.mek_global:getCharacterNameFromID(creator)
	end
...
function saveItemProperties(object, use, useData, move, moveData, pickup, pickupData)
	...
	if getElementParent(getElementParent(object)) == getResourceRootElement(getResourceFromName("mek_item-world")) then
		...
		if id then
			...
			local itemName = exports.mek_item:getItemName(
				tonumber(getElementData(object, "itemID")),
				getElementData(object, "itemValue")
			)
			...
			local permissions = {
				...
				move = move,
				pickup = pickup,
				useData = fromJSON(useData),
				moveData = fromJSON(moveData),
				pickupData = fromJSON(pickupData),
			}
