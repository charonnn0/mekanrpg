function loadOneWorldItem(id1)
	dbQuery(
			...
			if rows > 0 then
				for index, row in ipairs(res) do
					...
					if itemID < 0 then -- weapon
						...
						local modelid = exports.mek_item:getItemModel(itemID, itemValue)
						...
						local rx, ry, rz, zoffset = exports.mek_item:getItemRotInfo(itemID)
						local obj = createItem(
							id,
							itemID,
							itemValue,
							modelid,
						...
						if isElement(obj) then
							exports.mek_pool:allocateElement(obj, itemID, true)
							setElementDimension(obj, dimension)
							setElementInterior(obj, interior)
							setElementData(obj, "creator", creator)
							setElementData(obj, "createdDate", createdDate)
							...
							local permissions = {
								...
								pickup = permPickup,
								useData = permUseData,
								moveData = permMoveData,
								pickupData = permPickupData,
							}
		...
		mysql:getConnection(),
		"SELECT `id`, `itemid`, `itemvalue`, `x`, `y`, `z`, `dimension`, `interior`, `rx`, `ry`, `rz`, `creator`, `creationdate`, `protected`, `perm_use`, `perm_move`, `perm_pickup`, `perm_use_data`, `perm_move_data`, `perm_pickup_data` FROM `worlditems` WHERE `id` = ?",
		id1
	)
end
...
function loadWorldItems(res)
	...
	dbExec(
		...
		"DELETE FROM `worlditems` WHERE `protected`='0' AND `itemID` NOT IN(81, 103, 169) AND ((DATEDIFF(NOW(), creationdate) > 30) OR (DATEDIFF(NOW(), creationdate) > 7 AND `itemID` = 72)) "
	)
	...
	end, mysql:getConnection(), "SELECT `id` FROM `worlditems`")
	setTimer(restartResource, 5000, 1, getResourceFromName("mek_item-texture"))
end
