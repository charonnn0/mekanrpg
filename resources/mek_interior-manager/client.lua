...
function createIntManagerWindow(IntsList)
	...
	if IntsList then
		...
		for i = 1, 6 do
			...
			colAddress[i] = guiGridListAddColumn(targetList[i], "Address", 0.2)
		...
		for _, record in ipairs(IntsList) do
			...
			if record[13] ~= "0" then
				i = 5
				count[5] = count[5] + 1
			elseif isInteriorInactive then
				i = 2
				count[2] = count[2] + 1
			elseif record[12] == "1" then
				i = 3
				count[3] = count[3] + 1
			elseif record[7] == "1" then
				i = 4
				count[4] = count[4] + 1
			else
				count[1] = count[1] + 1
			...
			guiGridListSetItemText(
				...
				row,
				colOwner[i],
				charName(record[5]) .. cked(record[7]) .. accountName(record[6]) .. (record[20] or ""),
				false,
				false
			)
			guiGridListSetItemText(targetList[i], row, colLastUsed[i], lastUsed, false, false)
			guiGridListSetItemText(targetList[i], row, colAddress[i], record[21] or "", false, false)
			guiGridListSetItemText(
				targetList[i],
				row,
				colLocked[i],
				record[9] == "0" and "Unlocked" or "Locked",
				false,
				false
			)
			...
			guiGridListSetItemText(
				...
				row,
				colDeleted[i],
				record[13] == "0" and "No" or "by " .. record[13],
				false,
				false
			)
		...
		addEventHandler("onClientGUIClick", bSearch, function(button)
			if button == "left" then
				...
				local lText = guiCreateLabel(
					...
					341,
					16,
					"Input any related information about an interior (ID, Name, Owner, Price,...) :",
					false,
					wInteriorSearch
				)
		...
		addEventHandler("onClientGUIClick", bDelInt, function(button)
			if button == "left" then
				local row, col = -1, -1
				...
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						...
						triggerServerEvent("interiorManager.delint", localPlayer, gridID)
					else
		...
		addEventHandler("onClientGUIClick", bToggleInt, function(button)
			if button == "left" then
				...
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						local gridID = guiGridListGetItemText(activeList, row, 1)
						triggerServerEvent("interiorManager.disableInt", localPlayer, gridID)
					else
						guiSetText(gWin, "You need to select an interior from the list below first.")
					end
				end
		...
		addEventHandler("onClientGUIClick", bGotoInt, function(button)
			if button == "left" then
				local row, col = -1, -1
				local activeList = getListFromActiveTab(intsTabPanel)
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						local gridID = guiGridListGetItemText(activeList, row, 1)
						triggerServerEvent("interiorManager.gotoInt", localPlayer, gridID)
					else
						guiSetText(gWin, "You need to select an interior from the list below first.")
					end
				end
		...
		addEventHandler("onClientGUIClick", bRestoreInt, function(button)
			if button == "left" then
				local row, col = -1, -1
				local activeList = getListFromActiveTab(intsTabPanel)
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						local gridID = guiGridListGetItemText(activeList, row, 1)
						triggerServerEvent("interiorManager.restoreInt", localPlayer, gridID)
					else
						guiSetText(gWin, "You need to select an interior from the list below first.")
					end
				end
		...
		addEventHandler("onClientGUIClick", bRemoveInt, function(button)
			if button == "left" then
				local row, col = -1, -1
				local activeList = getListFromActiveTab(intsTabPanel)
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						local gridID = guiGridListGetItemText(activeList, row, 1)
						triggerServerEvent("interiorManager.removeInt", localPlayer, gridID)
					else
						guiSetText(gWin, "You need to select an interior from the list below first.")
					end
				end
		...
		addEventHandler("onClientGUIClick", bForceSell, function(button)
			if button == "left" then
				...
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						local gridID = guiGridListGetItemText(activeList, row, 1)
						triggerServerEvent("interiorManager.forceSellInt", localPlayer, gridID)
					else
						guiSetText(gWin, "You need to select an interior from the list below first.")
					end
				end
		...
		addEventHandler("onClientGUIClick", bAdminNote, function(button)
			if button == "left" then
				...
				if activeList then
					...
					if row ~= -1 and col ~= -1 then
						local gridID = guiGridListGetItemText(activeList, row, 1)
						triggerServerEvent("interiorManager.openAdminNote", localPlayer, gridID)
					else
						guiSetText(gWin, "You need to select an interior from the list below first.")
					end
				end
		...
		if noteId then
			...
			if editExiting then
				text = "Editing note #"
					...
					.. ". This note's creator will be transferred to you upon completion:"
				...
				text = "Viewing note #"
					...
					.. " by "
					.. currentNote.creatorname
					.. ". To edit, double LEFT click instead."
			end
...
function createCheckIntWindow(result, adminTitle, history, notes1, players)
	...
	guiSetInputEnabled(true)
	notes = notes1
	checkIntWindow = guiCreateWindow(
		sx / 2 - 300,
		sy / 2 - 233,
		600,
		466,
		"Interior Manager - Interior Admin Note - "
			.. adminTitle
			.. " "
			.. getElementData(localPlayer, "account_username"),
		false
	)
	...
	local lAddress = guiCreateLabel(12, 95, 365, 17, "Address: ", false, checkIntWindow)
	...
	local bCopyAdminInfo =
		guiCreateButton(110, 133, 220, 20, "Copy Your Admin Name & Current Date", false, checkIntWindow)
	...
	guiSetText(
		lOwner,
		"Owner: " .. charName(result[1][5]) .. cked(result[1][7]) .. accountName(result[1][6]) .. (result[1][22] or "")
	)
	...
	guiSetText(lAddress, "Address: " .. ((result[1][23] and result[1][24]) or ""))

	if history then
		for _, h in ipairs(history) do
			local row = guiGridListAddRow(gHistory)
			guiGridListSetItemText(gHistory, row, colDate, h[1] or "Bilinmiyor", false, true)
			guiGridListSetItemText(gHistory, row, colAction, h[2] or "Bilinmiyor", false, false)
			guiGridListSetItemText(gHistory, row, colActor, h[3] or "Bilinmiyor", false, false)
			guiGridListSetItemText(gHistory, row, colLogID, h[4] or "Bilinmiyor", false, false)
		end
	end
	...
	addEventHandler("onClientGUIClick", bToggleInt1, function(button)
		if button == "left" then
			triggerServerEvent("interiorManager.disableInt", localPlayer, result[1][1])
		end
	end, false)

	local bGotoInt1, bRestoreInt1, bDelInt1 = nil
	if result[1][15] ~= "0" then
		bRestoreInt1 = guiCreateButton(12, 430, 90, 28, "RESTORE INT", false, checkIntWindow)
		guiSetFont(bRestoreInt1, "default-bold-small")
		addEventHandler("onClientGUIClick", bRestoreInt1, function(button)
			if button == "left" then
				if triggerServerEvent("interiorManager.restoreInt", localPlayer, result[1][1]) then
		...
		addEventHandler("onClientGUIClick", bRemoveInt1, function(button)
			if button == "left" then
				if triggerServerEvent("interiorManager.removeInt", localPlayer, result[1][1]) then
					closeCheckIntWindow()
				end
			end
		end, false)
		...
		addEventHandler("onClientGUIClick", bGotoInt1, function(button)
			if button == "left" then
				triggerServerEvent("interiorManager.gotoInt", localPlayer, result[1][1])
			end
		end, false)
		bDelInt1 = guiCreateButton(112, 430, 90, 28, "DEL INT", false, checkIntWindow)
		guiSetFont(bDelInt1, "default-bold-small")
		addEventHandler("onClientGUIClick", bDelInt1, function(button)
			if button == "left" then
				if triggerServerEvent("interiorManager.delint", localPlayer, result[1][1]) then
					triggerServerEvent("interiorManager.checkint", localPlayer, localPlayer, "checkint", result[1][1])
				end
			end
		end, false)
