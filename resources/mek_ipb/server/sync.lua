addEvent("ipb.updateStats", true) -- Register event on server as well to be safe
...
function sendListenerStats(player, mode)
    ...
    if settings then
        triggerClientEvent(player, "ipb.updateStats", player, mode, getPerformanceStats(settings.category, settings.options, settings.filter))
    end
end
...
setTimer(updateListeners, UPDATE_FREQUENCY, 0)
