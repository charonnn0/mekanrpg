...
local function watchChanges()
	...
	then
		...
		for i = 1, #property do -- Remove the ugly Shadow
			texture[i] = dxCreateTexture(
				tostring("interiors/ID24-BusinessGarage/" .. property[i][1]),
				"argb",
				true,
				"wrap",
				"2d"
			...
			if texture[i] then
				if i <= 21 then
					shader[i], _ = dxCreateShader("interiors/ID24-BusinessGarage/shader.fx", 0, 0, false, "world")
				else
					shader[i], _ = dxCreateShader("interiors/ID24-BusinessGarage/shader.fx", 0, 0, false, "object")
				end
				if shader[i] then
