...
class("_thread")({
	...
	query = function(self, array, func)
		self:add(function()
			...
			local db = exports.mek_mysql:getConnection()

			db:query(function(queryHandle)
				local res, rows, err = queryHandle:poll(0)
				if rows > 0 then
					func(res, rows, err)
				else
					func(false)
	...
	prepare = function(self, array, table_name, func)
		self:add(function()
			...
			if db:exec(array) then
				...
				end, "SELECT id FROM " .. table_name .. " WHERE id = LAST_INSERT_ID()")
			end
			if self.debug then
