...
function interiorShowPickups(element)
	...
	if getElementType(element) == "elevator" then
		outsidePickup = createPickup(
			entrance[INTERIOR_X],
			entrance[INTERIOR_Y],
			entrance[INTERIOR_Z],
			3,
			int[INTERIOR_DISABLED] and 1314
				or (
					getElementType(element) == "elevator" and obj
					or (
						int[INTERIOR_TYPE] == 2 and obj
						or (
							int[INTERIOR_OWNER] < 1
								and int[INTERIOR_FACTION] < 1
								and (int[INTERIOR_TYPE] == 1 and 1272 or 1273)
							or obj
						)
					)
				)
		...
		if debugmode then
			outputDebugString(
				"interiorShowPickups returning with  " .. tostring(dbid) .. ": true, " .. getElementType(element) == "interior" and 1
					or 2
			)
		end
		return true, getElementType(element) == "interior" and 1 or 2
	else
		outsidePickup = entrance.x
			and createPickup(
				entrance.x,
				entrance.y,
				entrance.z,
				3,
				int.disabled and 1314
					or (
						getElementType(element) == "elevator" and obj
						or (
							int.type == 2 and obj
							or (int.owner < 1 and int.faction < 1 and (int.type == 1 and 1272 or 1273) or obj)
						)
					)
			)
...
function interiorRemovePickups(element)
	...
	if debugmode then
		outputDebugString(
			"interiorRemovePickups running with  " .. tostring(dbid) .. " " .. getElementType(element) == "elevator" and "(elevator)"
				or "(interior)"
		)
	end
...
addEventHandler(
	...
	getResourceRootElement(getResourceFromName("mek_elevator")),
	function(stoppedRes)
		clearElevators()
	end
)
...
function elevator_initializeSoFar(forced)
	for _, elevator in ipairs(getElementsByType("elevator")) do
		if
			forced
			or (not elevatorsToBeLoaded[elevator] and not elevatorsSpawned[getElementData(elevator, "dbid") or 0])
		then
...
addEventHandler("onClientResourceStart", getResourceRootElement(getResourceFromName("mek_interior")), function()
	for _, interior in ipairs(getElementsByType("interior")) do
		intsToBeLoaded[interior] = true
...
addEventHandler("onClientResourceStart", getResourceRootElement(getResourceFromName("mek_elevator")), function()
	for _, elevator in ipairs(getElementsByType("elevator")) do
		elevatorsToBeLoaded[elevator] = true
