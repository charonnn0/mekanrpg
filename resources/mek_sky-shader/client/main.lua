...
function startDynamicSky()
	...
	shaderTable.skyboxTropos = dxCreateShader("public/shaders/shader_dynamicSky2tropos.fx", 2, 0, false, "object")

	if dynamicSkySettings.stratosBottomStars then
		shaderTable.skyboxStratos = dxCreateShader("public/shaders/shader_dynamicSky2up.fx", 2, 0, false, "object")
	else
		shaderTable.skyboxStratos = dxCreateShader("public/shaders/shader_dynamicSky2stratos.fx", 2, 0, false, "object")
	end

	if dynamicSkySettings.stratosBottomStars then
		shaderTable.skyboxBottom = dxCreateShader("public/shaders/shader_dynamicSky2down.fx", 3, 0, false, "object")
	else
		shaderTable.skyboxBottom = dxCreateShader("public/shaders/shader_dynamicSky2bottom.fx", 3, 0, false, "object")
	end

	shaderTable.clear = dxCreateShader("public/shaders/shader_clear.fx", 3, 0, false, "world")
	textureTable.cloud = dxCreateTexture("public/images/clouds.dds", "dxt5")
	textureTable.normal = dxCreateTexture("public/images/clouds_normal.jpg", "dxt5")
	textureTable.skybox = dxCreateTexture("public/images/skybox.dds", "dxt5")
	moonPhase = getCurrentMoonPhase()
	textureTable.moon = dxCreateTexture("public/images/moon/" .. toint(20 - toint(moonPhase * 20)) .. ".png")
	...
	dxSetShaderValue(
		shaderTable.skyboxTropos,
		"gScale",
		dynamicSkySettings.modelScale[1] / dynamicSkySettings.origSizeMul,
		dynamicSkySettings.modelScale[2] / dynamicSkySettings.origSizeMul,
		dynamicSkySettings.modelScale[3] / dynamicSkySettings.origSizeMul
	)
	...
	if dynamicSkySettings.stratosBottomStars then
		dxSetShaderValue(shaderTable.skyboxBottom, "sCubeTex", textureTable.skybox)
	end
	...
	dxSetShaderValue(
		shaderTable.skyboxBottom,
		"gStratosFade",
		dynamicSkySettings.stratosFade[1],
		dynamicSkySettings.stratosFade[2]
	)
	dxSetShaderValue(
		shaderTable.skyboxBottom,
		"gScale",
	...
	modelTable.txd = engineLoadTXD("public/images/skybox_model.txd")
	engineImportTXD(modelTable.txd, dynamicSkySettings.modelID)
	modelTable.dff = engineLoadDFF("public/models/skybox_model.dff", dynamicSkySettings.modelID)
	engineReplaceModel(modelTable.dff, dynamicSkySettings.modelID, dynamicSkySettings.modelAlpha)
	...
	setObjectScale(
		modelTable.object,
		8 * dynamicSkySettings.origSizeMul,
		8 * dynamicSkySettings.origSizeMul,
		8 * dynamicSkySettings.origSizeMul
	)
	setElementAlpha(modelTable.object, 1)

	addEventHandler("onClientPreRender", root, renderSphere)
	addEventHandler("onClientPreRender", root, renderTime)
	shaderTable.isSwitched = false
	addEventHandler("onClientPreRender", root, switchShaders)
...
function stopDynamicSky()
	...
	removeEventHandler("onClientPreRender", root, renderSphere)
	removeEventHandler("onClientPreRender", root, renderTime)
	removeEventHandler("onClientPreRender", root, switchShaders)
...
function renderTime()
	...
	dxSetShaderValue(
		shaderTable.skyboxTropos,
		"gRotate",
		math.rad(dynamicSkySettings.sunPreRotation[1]),
		math.rad((timeAspect * 360) + dynamicSkySettings.sunPreRotation[2]),
		math.rad(dynamicSkySettings.sunPreRotation[3])
	)
	dxSetShaderValue(
		shaderTable.skyboxStratos,
		"gRotate",
	...
	dxSetShaderValue(
		shaderTable.skyboxTropos,
		"mRotate",
		math.rad(dynamicSkySettings.sunPreRotation[1] + dynamicSkySettings.moonPreRotation[1]),
		math.rad(
			((moonPhase + timeAspect) * 360)
				+ dynamicSkySettings.sunPreRotation[2]
				+ dynamicSkySettings.moonPreRotation[2]
		),
		math.rad(dynamicSkySettings.sunPreRotation[3] + dynamicSkySettings.moonPreRotation[3])
	)
	dxSetShaderValue(
		shaderTable.skyboxStratos,
		"mRotate",
	...
	dxSetShaderValue(
		shaderTable.skyboxTropos,
		"mMoonLightInt",
		math.sin(math.pi * moonPhase) * dynamicSkySettings.moonShine
	)
	dxSetShaderValue(
		shaderTable.skyboxStratos,
		"mMoonLightInt",
	...
	if thisWeather ~= oldWeather then
		...
		for index, value in ipairs(windVelocity) do
			if value[1] == thisWeather then
				windVelocityValue = value[2]
			end
		end
...
function getDynamicSunVector()
	local vecX, vecY, vecZ = eulerToVectorXY(
		math.rad(getNormalAngle(dynamicSkySettings.sunPreRotation[1])),
		math.rad(getNormalAngle((getTimeAspect() * 360) + dynamicSkySettings.sunPreRotation[2])),
		math.rad(getNormalAngle(dynamicSkySettings.sunPreRotation[3]))
	)
...
function getDynamicMoonVector()
	local vecX, vecY, vecZ = eulerToVectorXY(
		math.rad(getNormalAngle(dynamicSkySettings.sunPreRotation[1] + dynamicSkySettings.moonPreRotation[1])),
		math.rad(
			getNormalAngle(
				((getTimeAspect() + moonPhase) * 360)
					+ dynamicSkySettings.sunPreRotation[2]
					+ dynamicSkySettings.moonPreRotation[2]
			)
		),
		math.rad(getNormalAngle(dynamicSkySettings.sunPreRotation[3] + dynamicSkySettings.moonPreRotation[3]))
	)
	return vecX, vecY, vecZ
end
