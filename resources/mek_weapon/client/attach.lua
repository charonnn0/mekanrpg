...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 200, 200, "Silah Konum", false)
	...
	local weaponAttached = guiGridListAddColumn(weaponsGridList, "Durum", 0.2)
	addEventHandler("onClientGUIDoubleClick", weaponsGridList, doubleClickedWeapon, false)
	populateWeaponGridList()

	local help = guiCreateLabel(12, 155, 200, 10, "Eki açıp kapatmak için çift tıklayın.", false, sWin)
	guiSetFont(help, "default-small")

	btnSelectionEdit = guiCreateButton(0, 170, 90, 20, "Düzenle", false, sWin)
	addEventHandler("onClientGUIClick", btnSelectionEdit, createEditWindow, false)

	btnSelectionClose = guiCreateButton(100, 170, 90, 20, "Kapat", false, sWin)
	addEventHandler("onClientGUIClick", btnSelectionClose, closeSelectionWindow, false)
end
addCommandHandler("silahgoster", createSelectionWindow, false, false)
addCommandHandler("silahkonum", createSelectionWindow, false, false)
addCommandHandler("toggleattach", createSelectionWindow, false, false)
	...
	for i = 1, 12 do
		...
		if weapon ~= 0 then
			...
			guiGridListSetItemText(
				...
				row,
				3,
				getElementData(localPlayer, "attachingWeapon" .. weapon) and "Evet" or "Hayır",
				false,
				false
			)
		end
...
function createEditWindow()
	...
	local sx, sy = guiGetScreenSize()
	eWin = guiCreateWindow(sx - 250, sy - 375, 225, 325, "Silah Konum Düzenle", false)
	guiWindowSetSizable(eWin, false)

	guiCreateLabel(10, 26, 210, 15, "Silah: " .. name .. " (" .. id .. ") Skin: " .. skin, false, eWin)
	...
	addEventHandler("onClientGUIComboBoxAccepted", bonesComboBox, function()
	...
	for i, name in ipairs({ "PosX", "PosY", "PosZ", "RotX", "RotY", "RotZ" }) do
		...
		addEventHandler("onClientGUIChanged", posEditBox[i], function()
			local bone = guiComboBoxGetSelected(bonesComboBox) + 1
			local x, y, z, rx, ry, rz = getEditPositions()
			...
			if math.abs(x) > 0.5 or math.abs(y) > 0.5 or math.abs(z) > 0.5 then
				...
				if isElement(editingObject) then
					exports.mek_bones:attachElementToBone(editingObject, localPlayer, bone, x, y, z, rx, ry, rz)
				end
	...
	local help = guiCreateLabel(
		10,
		255,
		210,
		35,
		"X, Y ve Z uzaklıkları, -0.5 ile 0.5 arasında olmalıdır.\nÇevrenize bakmak için 'm' tuşuna basın.",
		false,
		eWin
	)
	guiSetFont(help, "default-small")
...
function createEditingObject(weapon, bone, x, y, z, rx, ry, rz)
	...
	exports.mek_bones:attachElementToBone(object, localPlayer, bone, x, y, z, rx, ry, rz)
...
function saveWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		table.insert(weaponTable[editingSkin], {
			editingWeapon,
			guiComboBoxGetSelected(bonesComboBox) + 1,
		...
		outputChatBox(
			"[!]#FFFFFF "
				.. getWeaponNameFromID(editingWeapon)
				.. " markalı silahın pozisyonu ["
				.. editingSkin
				.. "] ID'li skin için uygulandı.",
			0,
			255,
			0,
			true
...
addEventHandler("onClientPlayerWeaponSwitch", root, function(prev, curr)
	triggerServerEvent("destroyWeaponModel", localPlayer, getPedWeapon(localPlayer, curr))

	if
		getElementData(localPlayer, "attachingWeapon" .. getPedWeapon(localPlayer, prev))
		or forceToggle[getPedWeapon(localPlayer, prev)]
	then
		triggerServerEvent(
			"createWeaponModel",
			localPlayer,
			unpack(getAttachmentPositionMatrix(getPedWeapon(localPlayer, prev)))
		)
	end
end)
...
addEventHandler("onClientResourceStart", resourceRoot, function()
	...
	for i, model in pairs(models) do
		if getElementData(localPlayer, "attachingWeapon" .. i) or forceToggle[i] then
