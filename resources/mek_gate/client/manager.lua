...
function createGateManagerWindow(gatesList)
	...
	gWin = guiCreateWindow(sx / 2 - 400, sy / 2 - 300, 800, 600, "Gate Yöneticisi", false)
...
function createNewGate()
	if wCreateNewGate then
		...
		wCreateNewGate = guiCreateWindow(wx + 71, wy + 198, 658, 291, "Gate Manager - Create new gate", false)
		...
		exports.mek_global:guiComboBoxAdjustHeight(boxGateType, 5)
		...
		GUIEditor_Label[3] = guiCreateLabel(
			27,
			55,
			613,
			21,
			'Copy & paste whole line from .map file in 2 text boxes below. Example: <object id=""> ... </object>',
			false,
			wCreateNewGate
		)
...
function validation2()
	if
		guiGetText(eClosedState) ~= ""
		and guiGetText(eOpenedState) ~= ""
		and guiGetText(eTimeOut) ~= ""
		and guiGetText(eSpeed) ~= ""
	then
	...
	if button == "left" then
		...
		elseif source == bCreateGate then
			...
			triggerServerEvent(
				...
				tableOpened[8],
				tableOpened[9],
				(guiComboBoxGetSelected(boxGateType) + 1),
				...
				guiGetText(eAdminNote),
				(guiComboBoxGetItemText(boxGateSound, guiComboBoxGetSelected(boxGateSound))),
				guiGetText(eTriggerDistFoot),
				guiGetText(eTriggerDistVeh)
			)
		...
		elseif source == bYes then
			triggerServerEvent("delGate", localPlayer, localPlayer, "charon", gateID)
...
function validation()
	if
		tonumber(guiGetText(eObjectModel))
		and tonumber(guiGetText(eStartX))
		and tonumber(guiGetText(eStartY))
		and tonumber(guiGetText(eStartZ))
		and tonumber(guiGetText(eStartRX))
		and tonumber(guiGetText(eStartRY))
		and tonumber(guiGetText(eStartRZ))
		and tonumber(guiGetText(eEndX))
		and tonumber(guiGetText(eEndY))
		and tonumber(guiGetText(eEndZ))
		and tonumber(guiGetText(eEndRX))
		and tonumber(guiGetText(eEndRY))
		and tonumber(guiGetText(eEndRZ))
		and tonumber(guiGetText(eInterval))
		and tonumber(guiGetText(eSpeed))
		and tonumber(guiGetText(eInt))
		and tonumber(guiGetText(eDim))
		and tonumber(guiGetText(eInt))
	then
		guiSetEnabled(bSave, true)
	else
...
function doubleClickedGate(button, state)
	if button == "left" then
		if source == gatesGridList then
			...
			GUIEditor_Edit = {}
			local wx, wy = guiGetPosition(gWin, false)
			wGateEntryView = guiCreateWindow(wx + 70, wy + 190, 659, 316, "Gate Manager - Edit existing gate", false)
