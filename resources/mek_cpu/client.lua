...
local MED_CLIENT_CPU = 3
local MAX_CLIENT_CPU = 5
...
addCommandHandler("cpu", cpu, false, false)
...
function resStatRender()
	...
	if #clientRows ~= 0 then
		...
		_y = y
		dxDrawRectangle(x - 10, y, 150, height, tocolor(10, 10, 10, 200), true)
		y = y + 5
		for i, row in ipairs(clientRows) do
			if not disabledResources[row[1]] then
				...
				dxDrawText(text, x, y, 150, 15, tocolor(r, g, b, a), 1, font, "left", "top", false, false, true)
				y = y + 15
	...
	if #serverRows ~= 0 then
		...
		local y = sy / 2 - height / 2
		_y = y
		dxDrawRectangle(x - 10, y, 150, height + 15, tocolor(10, 10, 10, 200), true)
		y = y + 5
