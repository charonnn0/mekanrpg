local DEAGLE_WEAPON_ID = 24
local COOLDOWN_DURATION = 3000
local TAZER_RANGE = 35
...
local function getDeagleMode()
	local mode = getElementData(localPlayer, "deagle_mode")
	if not mode then
		triggerServerEvent("legal.setDeagleMode", localPlayer, 0)
		return 0
	end
	return mode
end

local function setDeagleMode(mode)
	triggerServerEvent("legal.setDeagleMode", localPlayer, mode)
	if mode == 0 then
		outputChatBox("[!]#FFFFFF Çok amaçlı tabancanızda tazer modunu aktifleştirdiniz.", 0, 0, 255, true)
	elseif mode == 1 then
		outputChatBox("[!]#FFFFFF Çok amaçlı tabancanızda öldürücü modunu aktifleştirdiniz.", 0, 0, 255, true)
	elseif mode == 2 then
		outputChatBox("[!]#FFFFFF Çok amaçlı tabancanızda radar modunu aktifleştirdiniz.", 0, 0, 255, true)
	end
end
...
local function handleModeSwitch()
	if
		not (
			...
			and getPedWeapon(localPlayer) == DEAGLE_WEAPON_ID
	...
	local currentMode = getDeagleMode()
...
local function enableTazerCooldown()
	...
	setElementData(localPlayer, "deagle_reload", true)

	if cooldownTimer then
		killTimer(cooldownTimer)
	end
	cooldownTimer = setTimer(disableTazerCooldown, COOLDOWN_DURATION, 1)
end

function disableTazerCooldown()
	cooldownActive = false
	toggleControl("fire", true)
	setElementData(localPlayer, "deagle_reload", false)
...
local function onClientWeaponFire(weaponID, ammo, ammoInClip, hitX, hitY, hitZ, hitElement)
	if weaponID ~= DEAGLE_WEAPON_ID then
		return
	end
	...
	if currentMode == 0 then
		...
		if distance < TAZER_RANGE then
			fxAddSparks(hitX, hitY, hitZ, 1, 1, 1, 1, 10, 0, 0, 0, true, 3, 1)
		end
		playSoundFrontEnd(38)
		triggerServerEvent("legal.tazerFired", localPlayer, hitX, hitY, hitZ, hitElement)
	end
end
...
local function onClientPlayerTarget(targetElement)
	...
	if getElementType(targetElement) == "vehicle" then
		if getPedWeapon(localPlayer) == DEAGLE_WEAPON_ID and getDeagleMode() == 2 then
			if getPedOccupiedVehicle(localPlayer) ~= targetElement then
				local actualSpeed = exports.mek_global:getVehicleVelocity(targetElement)
				local vehicleName = exports.mek_global:getVehicleName(targetElement)
				outputChatBox(
					">>#FFFFFF " .. vehicleName .. " markalı aracın hızı " .. math.floor(actualSpeed) .. " km/h.",
					0,
					0,
					255,
					true
...
local function onClientPlayerDamage(attacker, weaponID, bodypart, loss)
	if weaponID == DEAGLE_WEAPON_ID then
		local mode = getElementData(attacker, "deagle_mode")
...
local function showTazerEffect(x, y, z)
...
local function onResourceStart()
	...
	addEventHandler("onClientPlayerWeaponSwitch", root, disableTazerCooldown)
	addEvent("legal.showTazerEffect", true)
	addEventHandler("legal.showTazerEffect", root, showTazerEffect)
	getDeagleMode()
end
