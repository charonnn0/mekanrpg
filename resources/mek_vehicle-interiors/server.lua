...
addEventHandler("onResourceStop", resourceRoot, function()
	for i, vehicle in ipairs(getElementsByType("vehicle")) do
		if getElementData(vehicle, "entrance") then
			removeElementData(vehicle, "entrance")
		end
	end
end)
...
function add(vehicle)
	if getElementModel(vehicle) == 519 then
		...
		local gate = exports.mek_gate:createGate(
			2924,
			1,
			34.5,
			1199.8000488281,
		...
		return true, 1
	elseif getElementModel(vehicle) == 577 then
		local dbid = tonumber(getElementData(vehicle, "dbid"))
		local dim = dbid + 20000
		...
		return true, 1
	elseif getElementModel(vehicle) == 592 then
	...
	elseif getElementModel(vehicle) == 484 then
		addInterior(vehicle, 1.9, -3.2, 999.4, 2)
		return true, 2
	elseif getElementModel(vehicle) == 416 then
		local dim = tonumber(getElementData(vehicle, "dbid")) + 20000
		...
		addEventHandler("onColShapeLeave", marker, leaveVehicleEntrance)
		return true, 30
	elseif getElementModel(vehicle) == 427 then
		local dim = tonumber(getElementData(vehicle, "dbid")) + 20000
...
function enterCockpitByKey(thePlayer, key, keyState)
	...
	if vehicle then
		...
		if model == 577 or model == 519 then
			if
				getElementData(thePlayer, "duty_admin")
...
addEventHandler("enterVehicleInterior", root, function(vehicle)
	...
	if vehicles[vehicle] then
		if isVehicleLocked(vehicle) then
			outputChatBox("[!]#FFFFFF Araç kilitliyken içine girilemez.", client, 255, 0, 0, true)
		else
			local model = getElementModel(vehicle)
			...
			if model == 592 then
				if isPedInVehicle(client) then
					if getPedOccupiedVehicleSeat(client) == 0 then
						...
						setElementData(pedVehicle, "health", getElementHealth(pedVehicle))
						for i = 0, getVehicleMaxPassengers(pedVehicle) do
							local player = getVehicleOccupant(pedVehicle, i)
							if player then
						...
						setTimer(
							...
							1,
							pedVehicle,
							getElementInterior(exit),
							getElementDimension(exit),
							x,
							y,
							z
						)
		...
		if getElementData(value, "dbid") == dim then
			if isVehicleLocked(value) then
				...
				if getElementData(value, "airport.gate.connected") then
					...
					local teleportArr = exports.mek_sfia:getDataForExitingConnectedPlane(gateID, value)
					if teleportArr then
				...
				if getElementModel(value) == 454 then
					xadd, yadd, zadd = 0, 0, 4
				elseif getElementModel(value) == 508 then
				...
				elseif getElementModel(value) == 519 then
					local attached = getAttachedElements(value)
					for k, v in ipairs(attached) do
						if getElementType(v) == "colshape" then
							x, y, z = getElementPosition(v)
				...
				elseif getElementModel(value) == 577 then
					local attached = getAttachedElements(value)
					for k, v in ipairs(attached) do
						if getElementType(v) == "colshape" then
							x, y, z = getElementPosition(v)
				...
				elseif getElementModel(value) == 592 then
					local attached = getAttachedElements(value)
					for k, v in ipairs(attached) do
						if getElementType(v) == "colshape" then
							x, y, z = getElementPosition(v)
					...
					if isPedInVehicle(player) then
						if getPedOccupiedVehicleSeat(player) == 0 then
							...
							setTimer(
								...
								1,
								pedVehicle,
								getElementInterior(value),
								getElementDimension(value),
								x + xadd,
								y + yadd,
								z + zadd,
								rot
				...
				elseif getElementModel(value) == 416 then
					local attached = getAttachedElements(value)
					for k, v in ipairs(attached) do
						if getElementType(v) == "colshape" then
							x, y, z = getElementPosition(v)
				...
				elseif getElementModel(value) == 427 then
					local attached = getAttachedElements(value)
					for k, v in ipairs(attached) do
						if getElementType(v) == "colshape" then
							x, y, z = getElementPosition(v)
				...
				local teleportArr =
					{ x + xadd, y + yadd, z + zadd, getElementInterior(value), getElementDimension(value) }
				setElementInterior(player, teleportArr[4])
				setElementDimension(player, teleportArr[5])
				triggerClientEvent(player, "setPlayerInsideInterior2", player, teleportArr, false)
...
function isInPickup(thePlayer, thePickup, distance)
	if not isElement(thePickup) then
		return false
	end
	...
	return getDistanceBetweenPoints3D(ax, ay, az, bx, by, bz) < (distance or 2)
		and getElementInterior(thePlayer) == getElementInterior(thePickup)
		and getElementDimension(thePlayer) == getElementDimension(thePickup)
end
...
addEventHandler("onVehicleRespawn", root, function(blown)
		...
		if tpd > 20000 then
			...
			for key, value in ipairs(getElementsByType("vehicle")) do
				if getElementData(value, "dbid") == vid then
					exports.mek_global:sendLocalText(
						player,
						"* " .. getPlayerName(player):gsub("_", " ") .. " aracın kapısını çalmaya başlar.",
						255,
						51,
						102
					)
					exports.mek_global:sendLocalText(
						value,
						"* Aracın içinden vurma sesleri geliyor. ((" .. getPlayerName(player):gsub("_", " ") .. "))",
						255,
						51,
						102
					)
			...
			if vehicles[veh] then
				...
				if exit then
					...
					exports.mek_global:sendLocalText(
						exit,
						"* Dışarıdan kapıya vurma sesleri geliyor. (("
...
function exitVehicle(thePlayer, seat, jacked, door)
	if getElementModel(source) == 519 then
		if vehicles[source] then
			...
			local teleportArr = {
				...
				1200.3044433594,
				1,
				tonumber(getElementData(source, "dbid")) + 20000,
				0,
				0,
			}
			fadeCamera(thePlayer, false)
	...
	elseif getElementModel(source) == 577 then
		if vehicles[source] then
			removePedFromVehicle(thePlayer)
			local teleportArr =
				{ -388.79999, 53.9, 1225.80005, 1, tonumber(getElementData(source, "dbid")) + 20000, 0, 0 }
			fadeCamera(thePlayer, false)
			triggerClientEvent(thePlayer, "setPlayerInsideInterior2", source, teleportArr, false)
			fadeCamera(thePlayer, true)
			cancelEvent()
...
function seeThroughWindows(thePlayer, nx, ny, nz)
	local dim = getElementDimension(thePlayer)
	if
		(getElementData(thePlayer, "isInWindow") == false or not getElementData(thePlayer, "isInWindow"))
		and dim > 20000
	then
		outputChatBox("Viewing through windows", thePlayer)

		local id = dim - 20000
		local vehicle = exports.mek_pool:getElementByID("vehicle", tonumber(id))
		local x, y, z = getElementPosition(vehicle)
		...
		setElementData(thePlayer, "isInWindow", true)
		...
		local zoffset = 3
		if getVehicleModelFromName(getVehicleName(vehicle)) == 577 then
			zoffset = 8
		end

		attachElements(thePlayer, vehicle, 0, 0, zoffset)
	elseif getElementData(thePlayer, "isInWindow") == true then
...
function updateView(thePlayer)
	if getElementData(thePlayer, "isInWindow") == true then
		...
		if
			int ~= getElementData(thePlayer, "isInWindow:initialint")
			and dim ~= getElementData(thePlayer, "isInWindow:initialdim")
		then
