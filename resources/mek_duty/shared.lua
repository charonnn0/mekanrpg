...
function fetchAvailablePackages(targetPlayer)
	...
	local resource = getResourceRootElement(getResourceFromName("mek_faction"))
	if resource then
	...
	for factionID, _ in pairs(faction) do
		if factionDuty[factionID] then
			for i, factionPackage in pairs(factionDuty[factionID]) do
				local found = false
				for index, v in pairs(factionPackage[4]) do
				...
				if not found and veh then
					...
					for k, v in pairs(factionLocations[factionID]) do
						if tonumber(vehid) == tonumber(v[9]) then
							found = true
						end
					end
				end
