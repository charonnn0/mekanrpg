...
local function canEditObject(object)
	return exports.mek_integration:isAdminOnDuty(localPlayer)
		or exports.mek_integration:isPlayerServerManager(localPlayer)
end

addEvent("item:move", true)
addEventHandler("item:move", root, function(object)
	if not object then
		return
	end
	...
	if editing and object then
		...
		for k, name in ipairs({ "Model", "PosX", "PosY", "PosZ", "RotX", "RotY", "RotZ" }) do
			...
			addEventHandler("onClientGUIFocus", eInfo[k], function()
				ignoreKeyInput = true
			end, false)
			addEventHandler("onClientGUIBlur", eInfo[k], function()
				ignoreKeyInput = false
			end, false)
			addEventHandler("onClientGUIChanged", eInfo[k], function()
				if not ignoreGUIInput then
					local x, y, z =
						tonumber(guiGetText(eInfo[2])), tonumber(guiGetText(eInfo[3])), tonumber(guiGetText(eInfo[4]))
					local rx, ry, rz =
						tonumber(guiGetText(eInfo[5])), tonumber(guiGetText(eInfo[6])), tonumber(guiGetText(eInfo[7]))
		...
		if exports.mek_integration:isPlayerTrialAdmin(localPlayer) then
			...
			addEventHandler("onClientGUIClick", bProtect, function(button, state)
		...
		bReset = guiCreateButton(75, 200, 65, 20, "Sıfırla", false, wInfo)
		addEventHandler("onClientGUIClick", bReset, function(button, state)
			if button == "left" and state == "up" then
				reset()
				updateInfoPanel()
			end
		...
		addEventHandler("onClientGUIClick", bSave, function(button, state)
			if button == "left" and state == "up" then
				save()
				updateInfoPanel()
			end
		...
		local controls = guiCreateLabel(
			15,
			225,
			240,
			40,
			"Hareket: WASDF, Yön Yukarı/Aşağı\nRotasyon: Yön Sol/Sağ",
			false,
			wInfo
		)
...
addEventHandler("onClientElementDataChange", resourceRoot, function(name)
