...
function createHandlingWindow(mode)
	...
	local model = getElementData(theVehicle, "model") or 0
	local vehicle_shop_id = getElementData(theVehicle, "vehicle_shop_id")
	local vehName = year .. " " .. brand .. " " .. model
	local title = ""
	if mode == 0 then
		title = "#" .. vehicle_shop_id .. " numaralı (" .. vehName .. ") aracın tüm örnekleri"
	elseif mode == 1 then
		title = "Araç ID #" .. getElementData(theVehicle, "dbid") .. " (" .. vehName .. ") için Benzersiz Handling"
	elseif mode == 2 then
		title = "Araç İstatistikleri - " .. vehName
	else
		title = ""
	end
	...
	if not theVehicle then
		for i, theVehicle in ipairs(getElementsByType("vehicle")) do
			if getElementData(theVehicle, "vehicle_shop_id") == vehicle_shop_id then
				handlingCurrent = getVehicleHandling(theVehicle)
				break
			end
		end
	end
	...
	nameColumn = guiGridListAddColumn(handlingList, "Adı", 0.73)
	valueColumn = guiGridListAddColumn(handlingList, "Değeri", 0.2)

	maxVelocity = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, maxVelocity, nameColumn, "Maksimum Hız (Km/h)", false, false)
	guiGridListSetItemText(
		handlingList,
		maxVelocity,
		valueColumn,
		tostring(math.round(handlingCurrent["maxVelocity"], 0)),
		false,
		false
	)
	acceleration = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, acceleration, nameColumn, "Hızlanma", false, false)
	guiGridListSetItemText(
		handlingList,
		acceleration,
		valueColumn,
		tostring(math.round(handlingCurrent["engineAcceleration"], 1)),
		false,
		false
	)
	engineInertia = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, engineInertia, nameColumn, "Motor Gücü", false, false)
	guiGridListSetItemText(
		handlingList,
		engineInertia,
		valueColumn,
		tostring(math.round(handlingCurrent["engineInertia"], 1)),
		false,
		false
	)

	lowerLimit = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, lowerLimit, nameColumn, "Süspansiyon Yüksekliği", false, false)
	guiGridListSetItemText(
		handlingList,
		lowerLimit,
		valueColumn,
		tostring(handlingCurrent["suspensionLowerLimit"]),
		false,
		false
	)
	suspensionBias = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, suspensionBias, nameColumn, "Süspansiyon Önyargısı", false, false)
	guiGridListSetItemText(
		handlingList,
		suspensionBias,
		valueColumn,
		tostring(math.round(handlingCurrent["suspensionFrontRearBias"], 1)),
		false,
		false
	)
	suspensionForce = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, suspensionForce, nameColumn, "Süspansiyon Kuvveti", false, false)
	guiGridListSetItemText(
		handlingList,
		suspensionForce,
		valueColumn,
		tostring(math.round(handlingCurrent["suspensionForceLevel"], 3)),
		false,
		false
	)
	suspensionDamping = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, suspensionDamping, nameColumn, "Süspansiyon Sönümleme", false, false)
	guiGridListSetItemText(
		handlingList,
		suspensionDamping,
		valueColumn,
		tostring(math.round(handlingCurrent["suspensionDamping"], 5)),
		false,
		false
	)
	steeringLock = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, steeringLock, nameColumn, "Direksiyon Kilidi", false, false)
	guiGridListSetItemText(
		handlingList,
		steeringLock,
		valueColumn,
		tostring(math.round(handlingCurrent["steeringLock"], 1)),
		false,
		false
	)
	driveType = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, driveType, nameColumn, "Sürüş Tipi", false, false)
	...
	guiGridListSetItemText(handlingList, massWeight, nameColumn, "Araç Ağırlığı (Kg)", false, false)
	guiGridListSetItemText(
		handlingList,
		massWeight,
		valueColumn,
		tostring(math.round(handlingCurrent["mass"], 0)),
		false,
		false
	)
	massX = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, massX, nameColumn, "Ağırlık Merkezi X", false, false)
	guiGridListSetItemText(
		handlingList,
		massX,
		valueColumn,
		math.round(handlingCurrent["centerOfMass"][1], 2),
		false,
		false
	)
	massY = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, massY, nameColumn, "Ağırlık Merkezi Y", false, false)
	guiGridListSetItemText(
		handlingList,
		massY,
		valueColumn,
		math.round(handlingCurrent["centerOfMass"][2], 2),
		false,
		false
	)
	massZ = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, massZ, nameColumn, "Ağırlık Merkezi Z", false, false)
	guiGridListSetItemText(
		handlingList,
		massZ,
		valueColumn,
		math.round(handlingCurrent["centerOfMass"][3], 2),
		false,
		false
	)
	dragCoeff = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, dragCoeff, nameColumn, "Sürükleme Katsayısı", false, false)
	guiGridListSetItemText(
		handlingList,
		dragCoeff,
		valueColumn,
		tostring(math.round(handlingCurrent["dragCoeff"], 1)),
		false,
		false
	)
	brakeForce = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, brakeForce, nameColumn, "Fren Gücü", false, false)
	guiGridListSetItemText(
		handlingList,
		brakeForce,
		valueColumn,
		tostring(math.round(handlingCurrent["brakeDeceleration"], 1)),
		false,
		false
	)
	brakeBias = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, brakeBias, nameColumn, "Fren Eğilimi", false, false)
	guiGridListSetItemText(
		handlingList,
		brakeBias,
		valueColumn,
		tostring(math.round(handlingCurrent["brakeBias"], 1)),
		false,
		false
	)
	tracMultiply = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, tracMultiply, nameColumn, "Çekiş Çarpanı", false, false)
	guiGridListSetItemText(
		handlingList,
		tracMultiply,
		valueColumn,
		tostring(math.round(handlingCurrent["tractionMultiplier"], 2)),
		false,
		false
	)
	tracBias = guiGridListAddRow(handlingList)
	guiGridListSetItemText(handlingList, tracBias, nameColumn, "Çekiş Eğilimi", false, false)
	guiGridListSetItemText(
		handlingList,
		tracBias,
		valueColumn,
		tostring(math.round(handlingCurrent["tractionBias"], 2)),
		false,
		false
	)

	if mode == 0 then
		bReset = guiCreateButton(0.05, 0.925, 0.45, 0.05, "Varsayılanı Yükle", true, wHandling)
		addEventHandler("onClientGUIClick", bReset, function(button)
			if button == "left" then
		...
		bSave = guiCreateButton(0.5, 0.925, 0.45, 0.05, "Veritabanına Kaydet", true, wHandling)
	...
	elseif mode == 1 or mode == 2 then
		...
		addEventHandler("onClientGUIClick", btn["close"], function(button)
			if button == "left" then
				closeHandlingWindows()
			end
		end, false)
		...
		if mode == 1 then
			btn["reset"] = guiCreateButton(0.01 + (btnW * 1 + 0.005), 0.925, btnW, 0.05, "Sıfırla", true, wHandling)
			addEventHandler("onClientGUIClick", btn["reset"], function(button)
				if button == "left" then
					closeHandlingWindows()
					triggerServerEvent("handlingSystem:resetHandling", localPlayer, theVehicle, mode)
					triggerEvent("playSuccess", localPlayer)
			...
			addEventHandler("onClientGUIClick", btn["save"], function(button)
				if button == "left" then
...
addEvent("vehicleManager.editHandling", true)
addEventHandler("vehicleManager.editHandling", localPlayer, createHandlingWindow)
	...
	if saveToSQL == 1 or saveToSQL == 0 then
		...
		if not wEdit then
			...
			if (source == handlingList) and not (name == "") then
				...
				wEdit = guiCreateWindow(x, y, width, height, "Handling Düzenleme - " .. name, false)

				newValue = guiCreateEdit(0.03, 0.20, 2.0, 0.3, value, true, wEdit)

				finishEdit = guiCreateButton(0.6, 0.75, 0.4, 0.2, "Bitir", true, wEdit)
				addEventHandler("onClientGUIClick", finishEdit, newHandling)

				cancelEdit = guiCreateButton(0.05, 0.75, 0.4, 0.2, "İptal", true, wEdit)
...
function checkValues()
	for k, v in pairs(handlingValues) do
		...
		elseif name == "Drive Type" then
			if
				tostring(guiGetText(source)) == "rwd"
				or tostring(guiGetText(source)) == "fwd"
				or tostring(guiGetText(source)) == "awd"
			then
				guiSetEnabled(finishEdit, true)
			else
				guiSetEnabled(finishEdit, false)
...
function showConfirmReloadAllInstances(vehicle_shop_id, vehName)
	...
	GUIEditor_Window["reloadconfirmw"] = guiCreateWindow(
		x,
		y,
		width,
		height,
		"#" .. vehicle_shop_id .. " (" .. vehName .. ") modelinin tüm araç örneklerini yeniden yükle",
		false
	)

	GUIEditor_Label["reloadconfirml"] = guiCreateLabel(
		0.0383,
		0.1429,
		0.931,
		0.6468,
		"#"
			...
			.. " ("
			.. vehName
			.. ") araç mağazası ID'sine ait tüm araç örneklerinin handling ayarlarını değiştirdiniz.\n\nAncak, bu işlemden önce oyunda bulunan araçlar etkilenmeyecek, ta ki bir sonraki araç sistemi yeniden başlatılana kadar.\n\nBu modelin tüm araç örneklerini zorla yeniden yüklemek istiyor musunuz?\n\n* Tüm araç örneklerini yeniden yüklemek sunucuda büyük bir gecikmeye neden olabilir. Gereksiz yere tümünü yeniden yüklemekten kaçınmak için bunun yerine /reloadveh [Araç ID] komutunu kullanmayı deneyin. *",
		true,
		GUIEditor_Window["reloadconfirmw"]
	)
	guiLabelSetHorizontalAlign(GUIEditor_Label["reloadconfirml"], "left", true)

	GUIEditor_Button["reloadconfirm_ok"] = guiCreateButton(
		0.0172,
		0.8294,
		0.4808,
		0.127,
		"Tümünü yeniden yükle, ne yaptığımı biliyorum.",
		true,
		GUIEditor_Window["reloadconfirmw"]
	)
	addEventHandler("onClientGUIClick", GUIEditor_Button["reloadconfirm_ok"], function(button)
		if button == "left" then
			triggerServerEvent("vehicleManager.reloadAllInstancesOf", localPlayer, vehicle_shop_id)
			triggerEvent("playSuccess", localPlayer)
			closeConfirmReloadAllInstances()
		end
	end, false)

	GUIEditor_Button["reloadconfirml_cancel"] =
		guiCreateButton(0.4981, 0.8294, 0.4789, 0.127, "İptal", true, GUIEditor_Window["reloadconfirmw"])
	addEventHandler("onClientGUIClick", GUIEditor_Button["reloadconfirml_cancel"], function(button)
		if button == "left" then
			closeConfirmReloadAllInstances()
		end
	end, false)
...
function showDriveTestTimer(veh, driveTestTimeSec, thePed)
	...
	GUIEditor_Label["second"] = guiCreateLabel(
		5,
		22,
		305,
		18,
		"Bu test sürüşü sona ermesine " .. driveTestTimeSec .. " saniye var.",
		false,
		GUIEditor_Window["drivetest"]
	)
	...
	addEventHandler("onClientGUIClick", GUIEditor_Button["finish"], function()
		if source == GUIEditor_Button["finish"] then
			...
			setTimer(function()
				closeCountdownDriveTest()
				triggerServerEvent("vehicleManager.finishTestDrive", localPlayer, veh, thePed)
			end, 1000, 1)
			triggerEvent("playSuccess", localPlayer)

			setTimer(function()
				exports.mek_global:fadeFromBlack(source)
			end, 3000, 1)
		end
	end)
	if thePed then
		guiSetSize(GUIEditor_Window["drivetest"], width, height + 28, false)
		GUIEditor_Button["orderfinish"] =
			guiCreateButton(15, 46 + 28, 286, 26, "SİPARİŞ VER & BİTİR", false, GUIEditor_Window["drivetest"])
		addEventHandler("onClientGUIClick", GUIEditor_Button["orderfinish"], function()
			if source == GUIEditor_Button["orderfinish"] then
				exports.mek_global:fadeToBlack()
				setTimer(function()
					closeCountdownDriveTest()
					triggerServerEvent("vehicleManager.finishTestDrive", localPlayer, veh, thePed, true)
				end, 1000, 1)
				triggerEvent("playSuccess", localPlayer)
	...
	setTimer(function()
		exports.mek_global:fadeFromBlack()
	end, 1000, 1)
end
addEvent("vehicleManager.testdrivetimerGUI", true)
addEventHandler("vehicleManager.testdrivetimerGUI", localPlayer, showDriveTestTimer)
...
function killTestDrive()
	if realVeh then
		closeCountdownDriveTest()
		triggerServerEvent("vehicleManager.finishTestDrive", localPlayer, realVeh, thePed, false, true)
	end
end
