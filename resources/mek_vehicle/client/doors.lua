...
function openVehicleDoorGUI(vehicleElement)
	...
	vControlGUI["close"] = guiCreateButton(23, guiPos, 230, guiPos, "Kapat", false, vControlGUI["main"])
...
function updateServerSide(theScrollBar, state)
	if vControlGUI["main"] then -- and state == "up" then
		...
		if
			not (getPedOccupiedVehicle(localPlayer) == controllingVehicle)
			and not (getDistanceBetweenPoints3D(vehicle1x, vehicle1y, vehicle1z, player1x, player1y, player1z) < 5)
		then
			closeVehicleGUI()
			return
		end
