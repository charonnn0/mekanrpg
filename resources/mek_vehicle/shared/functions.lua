...
function isVehicleWindowUp(theVehicle, real)
	if hasVehicleWindows(theVehicle) then
		if (hasVehicleRoof(theVehicle)) or real then
			local windows = getElementData(theVehicle, "windows")
			if not windows then
				return true
			end
		end
...
function isCabriolet(theVehicle)
	local model = getElementModel(theVehicle)
	if cabriolet[model] then
		local variant, variant2 = getVehicleVariant(theVehicle)
		if cabriolet[model][1] == variant or cabriolet[model][2] == variant then
			return true
		end
	end
	return false
...
function getVehicleType(...)
	local vehicleType = _getVehicleType(...)
	if vehicleType == "" then
		return "Trailer"
	end
	return vehicleType
