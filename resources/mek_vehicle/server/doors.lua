function controlVehicleDoor(door, position)
	...
	if
		not (getPedOccupiedVehicle(client) == source)
		and not (getDistanceBetweenPoints2D(vehicle1x, vehicle1y, player1x, player1y) < 5)
	then
		return
	end
...
addEventHandler("vehicle:control:doors", root, controlVehicleDoor)
...
function controlRamp(theVehicle)
	...
	if not (exports.mek_item:hasItem(theVehicle, 117)) then
		outputChatBox("You need the item in the cars inventory before you can do this!", client, 255, 0, 0)
		return
	end

	if not getElementData(theVehicle, "handbrake") or not isElementFrozen(theVehicle) then
	...
	if not rampObject or not (isElement(rampObject)) then
		if getElementModel(theVehicle) == 578 then
			...
			rampObject = createObject(
				16644,
				vehiclePositionX + 0.37,
				vehiclePositionY - 15.41,
				vehiclePositionZ - 2.05,
				vehicleRotationX + 180,
				vehicleRotationY + 10,
				vehicleRotationZ + 90
			)
			attachElements(rampObject, theVehicle, 0.37, -15.4, -2.05, 180, 10, 90)
			setElementPosition(theVehicle, getElementPosition(theVehicle))
			setElementData(theVehicle, "vehicle:ramp:object", rampObject, false)
		end
	else
		destroyElement(rampObject)
		setElementData(theVehicle, "vehicle:ramp:object", nil, false)
	end
end
addEvent("vehicle:control:ramp", true)
addEventHandler("vehicle:control:ramp", root, controlRamp)
...
addEventHandler("vehicle.handbrake.lifted", root, checkRamp)
