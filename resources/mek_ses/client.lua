...
window = guiCreateWindow((rx - 295), (ry / 2 - 253 / 2), 293, 257, "Ses Sistemi", false)
...
button["remove"] = guiCreateButton(9, 158, 274, 25, "Sil", false, window)
...
addEventHandler("onClientGUIClick", root, function()
	...
	elseif source == button["place"] then
		if isURL() then
			...
			exports.mek_infobox:addBox("error", "Sesi açmak için URL eklemeniz gerekmektedir.")
		end
...
addEventHandler("onPlayerStartSpeakerBoxSound", root, function(player, url, inVehicle)
	...
	if inVehicle then
		...
		attachElements(speakerSound[player], vehicle, 0, 5, 1)
	end
end)
