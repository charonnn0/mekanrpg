...
class("_thread")({
	...
	iterate = function(self, from, to, func, callback)
		self:add(function()
			...
			if self.debug then
				print("[DEBUG]thread iterate: " .. (getTickCount() - a) .. "ms")
			end
			if callback then
				callback()
			end
	...
	foreach = function(self, array, func, callback)
		self:add(function()
			...
			if self.debug then
				print("[DEBUG]thread foreach: " .. (getTickCount() - a) .. "ms")
			end
			if callback then
				callback()
			end
	...
	foreach_pairs = function(self, array, func, callback)
		self:add(function()
			...
			if self.debug then
				print("[DEBUG]thread foreach_pairs: " .. (getTickCount() - a) .. "ms")
			end
			if callback then
				callback()
			end
