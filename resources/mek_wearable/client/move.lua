...
function wearable_recieve_position(ITEM_POSITION)
	if type(ITEM_POSITION) == "table" then
		...
		local ITEM_X, ITEM_Y, ITEM_Z, ITEM_RX, ITEM_RY, ITEM_RZ, ITEM_SCALE =
			ITEM_POSITION[1],
			ITEM_POSITION[2],
			ITEM_POSITION[3],
			ITEM_POSITION[4],
...
function moving_object(objectID, dbid)
	...
	self.activeSlot = "arrowicon.png"
	local _, _, X_ATTACH, Y_ATTACH, Z_ATTACH, RX_ATTACH, RY_ATTACH, RZ_ATTACH =
		exports.mek_bones:getElementBoneAttachmentDetails(object)
	exports.mek_bones:attachElementToBone(object, localPlayer, wearable_bone, 0, 0, 0, 0, 0, 0)
	...
	addEventHandler("onClientRender", root, drawnArtifacts)
	...
	for index, data in pairs(positions.images) do
		...
		if shownImage then
			...
			if SCREEN_X or SCREEN_Y then
				...
				if inArea(posX, posY, posW, posH) then
					dxDrawRectangle(posX, posY, posW, posH, tocolor(0, 0, 0, 100))
				end

				dxDrawImage(posX, posY, posW, posH, "public/images/" .. data["src"])
			end
		end
	end
end
function wearable_move_object(POSITION_VALUE)
	...
	for POSITION_OFFSET = 1, POSITION_ROWS do
		...
		elseif
			POSITION_OFFSET > 3
			and POSITION_OFFSET <= 6
			and (default_position[POSITION_OFFSET] > 360 or default_position[POSITION_OFFSET] < -360)
		then
			default_position[POSITION_OFFSET] = 0
		end
	end
