...
local ped = createPed(240, 204.009765625, -41.6708984375, 1001.8046875, 180)
ped.interior = 1
ped.dimension = 8
ped.frozen = true
ped:setData("name", "Enes Yıldız")
ped:setData("interaction", {
	callbackEvent = "onWearableShop",
	args = {},
	description = ped:getData("name"):gsub("_", " "),
})

addEvent("onWearableShop", true)
addEventHandler("onWearableShop", root, function()
	if not shownWShop then
		...
		preview_element = exports["mek_object-preview"]:createObjectPreview(
			tempObject,
			0,
			0,
			0,
...
setTimer(function()
	...
	dxDrawText("Aksesuar Mağazası", x, y, w + x, 25 + y, tocolor(255, 255, 255), 1, fonts.SFUIRegular.caption, "center", "center")
	...
	if preview_element then
		rotation = rotation + 2
		exports["mek_object-preview"]:setRotation(preview_element, 0, 0, rotation)
	end
				...
				if id >= currentRow and id <= latestRow then
					...
					if inArea(x, y + (index * 25), w, 25) then
						...
						if isKeyPressed("mouse1") and lastClick + 300 <= getTickCount() then
							lastClick = getTickCount()
							if
								data["allowed_factions"]
								and not data["allowed_factions"][getElementData(localPlayer, "faction")]
							then
								outputChatBox(
									"[!]#FFFFFF Seçilmeye çalışılan aksesuarı sadece belirli birlikler kullanabilir.",
									0,
									255,
									0,
									true
	...
	if inArea(x, y, w, 25) then
		...
		if isKeyPressed("mouse1") and lastClick + 300 <= getTickCount() then
			...
			if selectedWearable ~= 0 then
				...
				categoryIndex = 0
				if preview_element then
					exports["mek_object-preview"]:destroyObjectPreview(preview_element)
					destroyElement(tempObject)
				end
			end
		end
	end
	dxDrawText("Aksesuarı Satın Al", x, y, w + x, 25 + y, tocolor(255, 255, 255), 1, fonts.SFUIRegular.caption, "center", "center")
	...
	dxDrawText("Kapat", x, y, w + x, 25 + y, tocolor(255, 255, 255), 1, fonts.SFUIRegular.caption, "center", "center")
end, 0, 0)
