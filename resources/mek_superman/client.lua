...
local function canFly()
	return exports.mek_integration:isPlayerTrialAdmin(localPlayer) and getElementData(localPlayer, "duty_admin")
end
...
function Superman.onDataChange(dataName, oldValue)
	...
	if
		dataName == "superman"
		and isElement(source)
		and getElementType(source) == "player"
		and oldValue ~= getElementData(source, dataName)
		and oldValue == true
		and getElementData(source, dataName) == false
	then
...
function Superman.processControls()
	...
	if DirectionModule ~= 0 then
		Direction = Vector3D:new(
			SightDirection.x * Direction.y - SightDirection.y * Direction.x,
	...
	setElementVelocity(
		localPlayer,
		Direction.x + self.extraVelocity.x,
	...
	for player in iterateFlyingPlayers() do
		...
		if playerPos.z > 0 then
			local hit, hitX, hitY, hitZ, hitElement = processLineOfSight(
				...
				playerPos.x,
				playerPos.y,
				playerPos.z - LANDING_DISTANCE - 1,
				true,
				true,
				true,
				true,
